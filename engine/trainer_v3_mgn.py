...
class Trainer_v3_mgn(object):
    ...
    def forward(self, batch_graphs):
        ...
        for i in range(len(batch_graphs)):
            for k in batch_graphs[i].keys:
                batch_graphs[i][k] = batch_graphs[i][k].to(self.device)
            # => move all the tensors in batch_graph to device
        scores = self.model(batch_graphs)

        assert batch_graphs[0].y.cpu().tolist() == batch_graphs[-1].y.cpu().tolist()
        assert batch_graphs[0].batch.cpu().tolist() == batch_graphs[-1].batch.cpu().tolist()
        # returns 1-D tensors:
        return scores, batch_graphs[0].y, batch_graphs[0].batch
    ...
    def train(self, epoch):
        ...
        for batch_idx, batch_graphs in enumerate(self.train_loader):
            # print(batch_graph_body, batch_graph_face)
            data_time.update(time.time() - end)
            self.optimizer.zero_grad()
            scores, labels, batch_vec = self.forward(batch_graphs)
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            for i in tqdm(range(len(self.test_data))):
                graphs = self.test_data[i]
                # qidx_a, qidx_b = graphs[0].qid.tolist()[0], graphs[-1].qid.tolist()[0]
                assert graphs[0].imgidxs.tolist() == graphs[-1].imgidxs.tolist()
                assert graphs[0].qid.tolist()[0] == graphs[-1].qid.tolist()[0]
                # print(qidx_body, qidx_face)
                # assert qidx_a == qidx_b, (qidx_a, qidx_b)
                for i in range(len(graphs)):
                    for k in graphs[i].keys:
                        graphs[i][k] = graphs[i][k].to(self.device)
                scores = self.model(graphs).tolist()
                for score, imgidx in zip(scores, graphs[0].imgidxs.tolist()):
                    score_mat[graphs[0].qid.tolist()[0], int(imgidx)] = score
        
        distmat = -score_mat
        mkdir_if_missing(osp.join(self.logdir, 'distmat'))
        np.save(osp.join(self.logdir, 'distmat', 'epoch_{}'.format(epoch)), distmat)
