...
class Trainer_sample(object):

    def __init__(self, cfg, model, optimizer, loss, dataset):
        self.device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        print('trainer.py', self.device)
        self.model = model.to(self.device)
        self.model.train()
        self.optimizer = optimizer
        self.loss = loss
        self.dataset = dataset
        sampler = RandomSampler(self.dataset.graph_train, replacement=True, num_samples=640)
        ...
        self.checkpoint_dir = osp.join(cfg.OUTPUT_DIR, 'checkpoints')
        self.tensorboard_dir = osp.join(cfg.OUTPUT_DIR, 'tensorboard')
