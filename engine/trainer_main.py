...
class trainer_main(object):
    ...
    def run(self):
        ...
        print(t(), 'Elapsed {}'.format(elapsed))
        print(t(), 'Best result at epoch {}: mAP {}, Rank-1 {}'.format(best_epoch, best_mAP, best_rank1))
        self.writer.close()
    ...
    def test(self):
        ...
        distmat = -score_mat
        print(t(), 'Done, dist matrix:', distmat.shape)
        print(t(), 'Computing CMC and mAP ...')
        ...
        print(t(), "Results ----------")
        print(t(), "mAP: {:.1%}".format(np.mean(mAP)))
        print(t(), "CMC curve")
        for r in [1,5,10,20]:
            print(t(), "Rank-{:<3}: {:.1%}".format(r, cmc[r-1]))
        print(t(), "------------------")
