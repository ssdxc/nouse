...
class Trainer_v4(object):
    ...
    def __init__(self, cfg, model, optimizer, loss, dataset):
        ...
        self.frozen = False
        self.frozen_layers = ['input_mlp']
        # self.frozen_layers = ['input_mlp', 'node_projection']
    ...
    def run(self):
        ...
        for epoch in range(1, self.max_epoch + 1):
            ...
            if loss_avg < 0.0001 and self.frozen == False:
                self.frozen = True
        ...
        if self.frozen == True:
                for name, module in self.model.named_children():
                    if name in self.frozen_layers:
                        print('frozen ... ', name, module)
                        module.eval()
                        for p in module.parameters():
                            p.requires_grad = False
                    else:
                        print('open ... ', name, module)
                        module.train()
                        for p in module.parameters():
