...
def main():
    root = '/data4/xieqk/code/gitee/megcn/datasets/campus4k/reid_data'
    ...
    train_dir_face = osp.join(root, 'face', 'insightface', 'v2_new', 'train_results')
    query_dir_face = osp.join(root, 'face', 'insightface', 'v2_new', 'query_results')
    gallery_dir_face = osp.join(root, 'face', 'insightface', 'v2_new', 'gallery_results')

    # save_root = '/data4/xieqk/code/gitee/megcn/datasets/campus4k/graph_data/'
...
def get_face_feats(vidnames, dir_face):
    ...
    for vidname in tqdm(vidnames):
        json_name = vidname + '.json'
        face_data_dict = read_json(osp.join(dir_face, json_name))
        face_data = face_data_dict['data']

        feat_face = get_feat_select_and_size(face_data)
