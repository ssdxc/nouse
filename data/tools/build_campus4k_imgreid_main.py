...
def main():
    ...
    root = '/data/xieqk/exp/megcn/campus4k-imgreid'
    save_root = '/data/xieqk/exp/megcn/campus4k-imgreid/{}_insightface_n{}/'.format(reid_model, N)
...
def make_graph_raw(save_dir, q_npz_path, q_face_dir, g_npz_path, g_face_dir, is_train=False):
    ...
    with open(osp.join(save_dir, 'q_imgnames.txt'), 'w') as qf:
        qf.write('\n'.join(q_imgnames))
    with open(osp.join(save_dir, 'g_imgnames.txt'), 'w') as gf:
        gf.write('\n'.join(g_imgnames))
    ...
    np.save(osp.join(save_dir, 'distmat_body'), distmat_body)
    np.save(osp.join(save_dir, 'distmat_face'), distmat_face)

    num_full = N
    num_face = N // 2

    # if not is_train:
    #     with open(osp.join(save_dir, 'rank.txt'), 'w') as rf:
    #         for i, indexes in enumerate(indices_body):
    #             q_name = q_imgnames[i]
    #             g_names = g_imgnames[indexes[:num_full]]
    #             rf.write('{}, {}'.format(q_name, ' '.join(g_names)))
