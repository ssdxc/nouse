...
def main():
    root = '/data/xieqk/data/campus4k'
    finetune_dir = '/data4/xieqk/log/arcface_torch/campus4k_imgreid/25.2_38.1'
    train_feats_body = osp.join(root, 'features_imgreid', 'fast-reid', 't_feats_body.npz')
    query_feats_body = osp.join(root, 'features_imgreid', 'fast-reid', 'q_feats_body.npz')
    gallery_feats_body = osp.join(root, 'features_imgreid', 'fast-reid', 'g_feats_body.npz')
    train_dir_face = osp.join(finetune_dir, 'train')
    query_dir_face = osp.join(finetune_dir, 'query')
    gallery_dir_face = osp.join(finetune_dir, 'gallery')
    # train_dir_face = osp.join(root, 'features', 'face/retina_arcface_0.7', 'train')
    # query_dir_face = osp.join(root, 'features', 'face/retina_arcface_0.7', 'query')
    # gallery_dir_face = osp.join(root, 'features', 'face/retina_arcface_0.7', 'gallery')

    save_root = '/data/xieqk/data/campus4k/exp_imgreid/megcn/v3_body_retina_full_n300_finetune/'
    # save_root = '/data/xieqk/data/campus4k/exp_imgreid/megcn/v3_body_retina_0.7_n300/'
...
def make_graph_raw(save_dir, q_npz_path, q_face_dir, g_npz_path, g_face_dir, is_train=False):
    ...
    # del q_body_torch, g_body_torch, distmat_body_torch
    ...
    # del q_face_torch, g_face_torch, distmat_face_torch
    # torch.cuda.empty_cache()
