...
def main():
    ...
    train_feats_body = osp.join(root, 'detections', 'merge', 't_feats_body.npz')
    query_feats_body = osp.join(root, 'detections', 'merge', 'q_feats_body.npz')
    gallery_feats_body = osp.join(root, 'detections', 'merge', 'g_feats_body.npz')
    train_dir_face = osp.join(root, 'detections', 'retina_arcface_0.7', 'train')
    query_dir_face = osp.join(root, 'detections', 'retina_arcface_0.7', 'query')
    gallery_dir_face = osp.join(root, 'detections', 'retina_arcface_0.7', 'gallery')
    head_dir_train = osp.join(root, 'detections', 'yolov5_head', 'train')
    head_dir_query = osp.join(root, 'detections', 'yolov5_head', 'query')
    head_dir_gallery = osp.join(root, 'detections', 'yolov5_head', 'gallery')

    save_root = '/data/xieqk/data/VC-Clothes/exp/graph/v3_body_retina0.7_head_n300/'
    train_save_dir = osp.join(save_root, 'train')
    test_save_dir = osp.join(save_root, 'test')

    make_graph_raw(train_save_dir, train_feats_body, train_dir_face, head_dir_train, train_feats_body, train_dir_face, head_dir_train, is_train=True)
    make_graph_raw(test_save_dir, query_feats_body, query_dir_face, head_dir_query, gallery_feats_body, gallery_dir_face, head_dir_gallery)


def make_graph_raw(save_dir, q_npz_path, q_face_dir, q_head_dir, g_npz_path, g_face_dir, g_head_dir, is_train=False):
    ...
    q_feats_face = get_face_head_feats(q_imgnames, q_face_dir, q_head_dir)
    ...
    g_feats_face = get_face_head_feats(g_imgnames, g_face_dir, g_head_dir)
