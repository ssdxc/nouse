...
def main():
    root = '/data/xieqk/data/campus4k/exp'

    train_feats_body = osp.join(root, 'reid_data', 'body', 'resnet50_campus4k_softmax', 'train_result.npz')
    query_feats_body = osp.join(root, 'reid_data', 'body', 'resnet50_campus4k_softmax', 'query_result.npz')
    gallery_feats_body = osp.join(root, 'reid_data', 'body', 'resnet50_campus4k_softmax', 'gallery_result.npz')

    train_dir_face = osp.join(root, 'face', 'retina_arcface_0.7', 'train')
    query_dir_face = osp.join(root, 'face', 'retina_arcface_0.7', 'retina_arcface_0.7', 'query')
    gallery_dir_face = osp.join(root, 'face', 'retina_arcface_0.7', 'retina_arcface_0.7', 'gallery')
    # train_dir_face = osp.join(root, 'reid_data/face', 'insightface/v2_new', 'train_results')
    # query_dir_face = osp.join(root, 'reid_data/face', 'insightface/v2_new', 'query_results')
    # gallery_dir_face = osp.join(root, 'reid_data/face', 'insightface/v2_new', 'gallery_results')

    save_root = '/data/xieqk/data/campus4k/exp/megcn/v2_body_retina0.7_n20/'
    # save_root = '/data/xieqk/data/campus4k/exp/megcn/v2_body_insightface_n50/'
    ...
    for i, vidname in enumerate(tqdm(q_vidnames)):
        ...
        if is_train:
            if _matches.sum() == num_full or _matches.sum() == 0:
                print('skip {}, matches_sum: {}, num_full: {}'.format(vidname, _matches.sum(), num_full))
                continue
        ...
        imglist.append('{}\n'.format(vidname))
        np.savez(
            osp.join(save_dir_raw, '{}_n{}.npz'.format(vidname, num_full)),
            imgidxs = _idxs, 
            matches = _matches,
            scores = _sims,
...
def get_face_feats(vidnames, dir_face):
    ...
    for vidname in tqdm(vidnames):
        ...
        if len(tid_feats) == 0:
            for json_file in face_files:
                face_data = read_json(json_file)
                feat = face_select_more(face_data)
                if feat is None:
                    continue
                tid_feats.append(feat)
...
def face_select(face_data, size=32, lambda_x=1.5, lambda_y=1.5):
...
def face_select_more(face_data, lambda_x=1.5, lambda_y=1.5):
    bboxs = face_data['bboxs']
    xc_mean, yc_mean = 0.478, 0.148
    xc_std, yc_std = 0.174, 0.122
    h, w = face_data['height'], face_data['width']
...
def get_face_head_feats(imgnames, dir_face, dir_head):
    feats = []
